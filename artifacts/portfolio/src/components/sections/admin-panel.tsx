...
function IconByKey({ k, size = 18, className }: { k: string; size?: number; className?: string }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/65 backdrop-blur-sm p-2 sm:p-6 md:p-10 overflow-y-auto">
      <div className="relative mx-auto w-full max-w-6xl bg-[#16171e] border border-[#44475a]/70 rounded-2xl shadow-[0_30px_90px_rgba(0,0,0,.6)] overflow-hidden">
        ...
        <div className="flex items-center justify-between gap-3 p-4 md:p-5 border-b border-[#44475a]/50 bg-[linear-gradient(135deg,rgba(109,179,63,.16),rgba(40,42,54,.6))]">
          <div>
            ...
            <p className="text-muted-foreground text-xs md:text-sm mt-0.5">
              Os dados são salvos em <code className="bg-[#191a21] px-1.5 py-0.5 rounded border border-[#44475a]/60">localStorage</code> e refletidos no portfólio em tempo real (como GIF).
            </p>
          </div>
          <div className="flex items-center gap-2 flex-wrap justify-end">
            {flash && (
              <span className="text-[11px] font-mono font-semibold text-[#50fa7b] bg-[#0e1c14] border border-[#50fa7b]/35 px-2.5 py-1 rounded-lg">
                ✓ {flash}
              </span>
            )}
            <button
              ...
              className="flex items-center gap-1.5 text-[11px] font-semibold rounded-lg px-2.5 py-1.5 border border-[#44475a]/70 text-muted-foreground hover:text-white hover:border-white/40"
            >
            ...
            <button
              type="button"
              onClick={onClose}
              className="flex items-center gap-1 rounded-lg w-9 h-9 justify-center border border-[#44475a]/70 text-muted-foreground hover:text-white hover:border-white/40"
        ...
        {/* Tabs */}
        <div className="flex gap-1.5 md:gap-2 px-3 md:px-4 py-3 border-b border-[#44475a]/40 overflow-x-auto bg-[#14151b]">
    ...
    <div>
      <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 rails-button rounded-lg px-3 py-1.5 text-xs font-semibold"
        >
...
function EmptyHint({ text, onAdd }: { text: string; onAdd: () => void }) {
  return (
    <div className="border border-dashed border-[#44475a]/70 rounded-xl p-4 flex items-center justify-between gap-3 bg-[#14151b]">
      <span className="text-sm text-muted-foreground">{text}</span>
      <button type="button" onClick={onAdd} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-white/5 border border-white/10 text-xs font-semibold">
...
function ProjectRow({
  ...
  p: AdminProject; index: number; total: number;
  ...
  return (
    <div className="border border-[#44475a]/50 bg-[#191a21] rounded-xl overflow-hidden">
      <div className="flex items-center justify-between gap-2 px-3 py-2 border-b border-[#44475a]/40 bg-[#14151b]/60">
        ...
        <div className="px-3 py-2.5 grid md:grid-cols-3 gap-3 text-[12px]">
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {p.tech.map((t) => <span key={t} className="px-1.5 py-0.5 rounded border border-[#44475a]/60 text-[10.5px] font-mono">{t}</span>)}
            </div>
          </div>
          <div>
    ...
    <div className="grid md:grid-cols-2 gap-3">
      ...
      <Field label="Prévia do ícone / tone" span={2}>
        <div className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 border ${draft.tone}`} style={{ background: "color-mix(in srgb, var(--tech-color,#fff) 12%, #101117)", boxShadow: "inset 0 0 0 1px color-mix(in srgb, var(--tech-color,#fff) 35%, transparent)" }}>
          <IconByKey k={draft.icon} size={22} />
          <div>
      ...
      <div className="md:col-span-2 flex items-center justify-end gap-2 pt-1">
        ...
        <button type="button" onClick={onSave} className="rails-button px-3 py-1.5 rounded-lg text-xs font-semibold inline-flex items-center gap-1.5">
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between mb-1">
        ...
        <button type="button" onClick={startAdd} className="inline-flex items-center gap-1.5 rails-button rounded-lg px-3 py-1.5 text-xs font-semibold"><Plus size={13} /> Nova experiência</button>
      </div>
      ...
      {draft && editingId && !data.experiences.some((x: AdminExperience) => x.id === editingId) && (
        <div className="border border-dashed border-[#84cc16]/50 rounded-xl p-3 bg-[#10140e]">
          <div className="text-xs font-bold text-[#84cc16] mb-2 font-mono uppercase">⬇ Rascunho — nova experiência</div>
      ...
      {data.experiences.map((e: AdminExperience, idx: number) => (
        <div key={e.id} className="border border-[#44475a]/50 bg-[#191a21] rounded-xl overflow-hidden">
          <div className="flex items-center justify-between gap-2 px-3 py-2 border-b border-[#44475a]/40 bg-[#14151b]/60">
            <div className="min-w-0">
              <div className="text-white font-semibold truncate">
...
function ExperienceForm({
  ...
  return (
    <div className="grid md:grid-cols-2 gap-3">
      ...
      <div className="md:col-span-2 flex items-center justify-end gap-2 pt-1">
        <button type="button" onClick={onCancel} className="px-3 py-1.5 rounded-lg text-xs font-semibold border border-[#44475a]/70 text-muted-foreground hover:text-white hover:border-white/40">Cancelar</button>
        <button type="button" onClick={onSave} className="rails-button px-3 py-1.5 rounded-lg text-xs font-semibold inline-flex items-center gap-1.5"><Save size={13} /> Salvar experiência</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="grid md:grid-cols-2 gap-4">
      ...
      <div className="md:col-span-2 flex justify-end gap-2">
        <button type="button" onClick={saveAbout} className="rails-button px-3 py-1.5 rounded-lg text-xs font-semibold inline-flex items-center gap-1.5"><Save size={13} /> Salvar seção Sobre</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={startAdd} className="rails-button rounded-lg px-3 py-1.5 text-xs font-semibold inline-flex items-center gap-1.5"><Plus size={13} /> Nova tecnologia</button>
      </div>
            ...
            {editingId === s.id && draft && (
              <div className="p-3 border-t border-[#44475a]/50 grid grid-cols-2 gap-3">
                ...
                <div className="col-span-2 flex justify-end gap-2">
                  ...
                  <button type="button" onClick={persist} className="rails-button px-2.5 py-1 rounded-md text-[11px] font-semibold inline-flex items-center gap-1"><Save size={12} /> Salvar</button>
                </div>
              </div>
            )}
          </div>
        ...
        {editingId && draft && !stack.some((x) => x.id === editingId) && (
          <div className="md:col-span-2 border border-dashed border-[#84cc16]/50 rounded-xl p-3 bg-[#10140e]">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2 flex justify-end gap-2">
                <button type="button" onClick={cancel} className="px-2.5 py-1 rounded-md text-[11px] font-semibold border border-[#44475a]/70 text-muted-foreground hover:text-white">Cancelar</button>
                <button type="button" onClick={persist} className="rails-button px-2.5 py-1 rounded-md text-[11px] font-semibold inline-flex items-center gap-1"><Save size={12} /> Criar tecnologia</button>
              </div>
            </div>
          </div>
        )}
