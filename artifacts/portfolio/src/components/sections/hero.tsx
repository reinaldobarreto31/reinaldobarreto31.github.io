...
import { SiPostgresql, SiDocker, SiGithub, SiJava, SiSpringboot, SiKotlin, SiFlutter } from "react-icons/si";
...
const STACK = [
  { label: "Java", icon: <SiJava size={13} />, tone: "tech-java" },
  { label: "Spring Boot", icon: <SiSpringboot size={13} />, tone: "tech-spring" },
  { label: "Kotlin", icon: <SiKotlin size={13} />, tone: "tech-kotlin" },
  ...
  return <section id="hero" ref={sceneRef} className="relative min-h-[100dvh] flex items-center justify-center pt-20 pb-16 md:pb-0 overflow-hidden">
    ...
    <div className="container mx-auto px-4 z-20 grid md:grid-cols-2 gap-12 items-center">
      <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .7 }} className="flex flex-col gap-6">
        <div className="rails-badge inline-flex items-center gap-3 px-4 py-2 rounded-full w-fit text-sm font-mono tracking-tight"><SiSpringboot className="text-[#6db33f] text-lg shrink-0" /><span className="rails-shine font-bold">Java · Spring Boot · Kotlin · Flutter</span></div>
        ...
        <p className="text-lg md:text-xl text-muted-foreground max-w-lg leading-relaxed" data-testid="text-hero-description">Desenvolvedor <span className="text-primary font-bold font-mono">Java &amp; Spring Boot</span> — com <span className="text-[#7f52ff] font-bold font-mono">Kotlin (Android)</span> em segundo lugar e <span className="text-[#02569b] font-bold font-mono">Flutter / Dart</span> em terceiro para mobile cross-platform. APIs REST escaláveis, microsserviços e apps nativos &amp; multiplataforma de confiança.</p>
      ...
      <motion.div initial={{ opacity: 0, scale: .92 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: .9, delay: .15 }} className="relative flex min-h-[25rem] items-center justify-center md:justify-end">
        ...
        <motion.div animate={{ y: [0, -18, 4, -12, 0], x: [0, 8, -5, 10, 0], rotateY: [0, 28, -14, 30, 0], rotateZ: [-4, 10, -6, 8, -4], scale: [1, 1.12, 0.96, 1.08, 1] }} transition={{ duration: 6.5, repeat: Infinity, ease: "easeInOut" }} className="ruby-gem ruby-gem-large absolute top-0 right-0 md:right-2 z-20"><SiSpringboot className="text-5xl" /></motion.div>
        <motion.div animate={{ y: [0, 16, -6, 12, 0], x: [0, -10, 6, -8, 0], rotateY: [0, -22, 12, -26, 0], rotateZ: [3, -8, 14, -5, 3], scale: [1, 1.15, 0.94, 1.1, 1] }} transition={{ duration: 7.4, repeat: Infinity, ease: "easeInOut", delay: .8 }} className="ruby-gem ruby-gem-rails absolute bottom-4 left-0 md:left-2 z-20"><SiKotlin className="text-3xl" /></motion.div>
        <motion.div animate={{ y: [0, -14, 6, -8, 2, 0], x: [0, 6, -8, 4, -6, 0], rotateZ: [0, 18, -12, 22, -8, 0], scale: [1, 1.2, 0.92, 1.14, 0.98, 1] }} transition={{ duration: 5.8, repeat: Infinity, ease: "easeInOut", delay: .2 }} className="ruby-gem ruby-gem-small absolute top-20 left-1 md:left-4 z-20"><SiFlutter className="text-2xl" /></motion.div>
        <div className="absolute bottom-3 right-0 bg-card/90 border border-border px-3 py-2 rounded-md shadow-lg flex items-center gap-2 z-20 backdrop-blur-sm"><span className="w-2 h-2 rounded-full bg-[#50fa7b] animate-pulse" /><span className="text-xs text-muted-foreground font-mono">dev.status: ready · Java + Spring + Kotlin + Flutter</span></div>
      </motion.div>
    </div>
